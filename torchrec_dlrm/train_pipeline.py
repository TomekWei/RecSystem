...
class TrainPipelineSparseDist(TrainPipeline[In, Out]):
    ...
    def __init__(
        ...
        #for torchvis
        self._iteration = 0
        self._visualize_iter = 50 
        self._loaddata_times = []
        self._forward_times = []
        self._backward_times = []
        self._optimizer_times = []
        self._other_times = []
        #
    ...
    def progress(self, dataloader_iter: Iterator[In]) -> Tuple[torch.Tensor, Out]:
        ...
        #load data
        loaddata_start = timeit.default_timer()
        ...
        torch.cuda.synchronize()
        #synchonize
        loaddata_end = timeit.default_timer()
        self._loaddata_times.append(loaddata_end - loaddata_start)
        #load data
        ...
        self._iteration += 1
        # if self._iteration == 70:
        #     output_path = f"/u/twei1/Projects/DLRCs/RecSystem/torchrec_dlrm/computationGragh/forwardd_model_graph_iter_{self._iteration}"
        #     graph = make_dot(losses, params=dict(self._model.named_parameters()), show_attrs=True, show_saved=True )
        ...
        # torch.cuda.synchronize()
            #synchonize
        ##Tong: will wait here
        other_start = timeit.default_timer()
        self._sparse_data_dist(self._batch_ip1, event)
            #synchonize
        torch.cuda.synchronize()
            #synchonize
        other_end = timeit.default_timer()
        self._other_times.append(other_end - other_start)
        torch.cuda.synchronize()
        forward_end = timeit.default_timer()
        self._forward_times.append(forward_end - forward_start)
        ##for torchvis
        ##Tong: will wait here 
        ...
        if self._model.training:
            ...
            self._backward_times.append(backward_end - backward_start)
            ...
            with record_function("## optimizer ##"):
                ...
                self._optimizer_times.append(optimizer_end - optimizer_start)
        # print(f"iteration: {self._iteration}")
        if self._iteration==1599:
            # output_path = f"/u/twei1/Projects/DLRCs/RecSystem/torchrec_dlrm/computationGragh/forwardd_model_graph_iter_{self._iteration}"
            # graph = make_dot(losses, params=dict(self._model.named_parameters()), show_attrs=True, show_saved=True )
            # graph.render(output_path, format="png")
            avg_loaddata_time = sum(self._loaddata_times) / len(self._loaddata_times) *1000
            avg_forward_time = sum(self._forward_times) / len(self._forward_times) *1000
            avg_backward_time = sum(self._backward_times) / len(self._backward_times) *1000
            avg_optimizer_time = sum(self._optimizer_times) / len(self._optimizer_times) *1000
            avg_other_time = sum(self._other_times) / len(self._other_times) *1000
            with open("DATA_PARALLEL_average_times.txt", "w") as f:
                f.write(f"Average Data Loading Time: {avg_loaddata_time:.5f} ms\n")
                f.write(f"Average Forward Pass Time: {avg_forward_time:.5f} ms\n")
                f.write(f"Average Backward Pass Time: {avg_backward_time:.5f} ms\n")
                f.write(f"Average Optimizer Time: {avg_optimizer_time:.5f} ms\n")
                f.write(f"Average Other Time: {avg_other_time:.5f} ms\n")
        ...
        return losses.mean().item(), output
    ...
    def _sparse_data_dist(
        ...
        with record_function("## sparse_data_dist ##"):
            with torch.cuda.stream(self._data_dist_stream):
                ##Tong: waiting for all data have been copied to GPU
                _wait_for_batch(batch, self._memcpy_stream)
                # ensure event in default stream is called before starting data dist
                if event is not None:
                    #Tong: event is the forward event, need to wait for it to finish
                    # pyre-ignore[16]
                    self._data_dist_stream.wait_event(event) # type: ignore

                # Tong: start data distribution
